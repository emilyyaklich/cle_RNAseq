...
# plot the upset 
png("plots/5h_up_AND_down.png", width=900, height=700)
...
# combine the two lists of data frames 
mydataSig_24h<-mapply(rbind, dataSigup_24h, dataSigdown_24h,SIMPLIFY=FALSE)

mydataSig_24h$result_AtCLV3p_S5L_24h
# rename the dataframes in the list
names(mydataSig_24h) <-c("AsterCLV3_24h", "AsterCLV3_L5S_24h","AtCLV3p_24h", "AtCLV3p_S5L_24h")

# subset the dataframes to only contain the gene ID and the direction of expression
gene_direction_only<-lapply(mydataSig_24h, function(x) {x[c("Gene", "Direction")]})
gene_direction_only<-gene_direction_only %>% purrr::reduce(full_join,by=c('Gene'))
# combine all three columns of direction into one string with where direction 
# is outlined. For example. "Up Up Down" If the gene is not in a treatment NA is used
gene_direction_only$dif<-paste(gene_direction_only$Direction.x,gene_direction_only$Direction.y,gene_direction_only$Direction.x.x,gene_direction_only$Direction.y.y, gene_direction_only$Direction)
...
# in our initial dataframe containing all treatments, add the directionality of the gene
mydataSig_24h$AsterCLV3_24h$Direction<-up_or_down$x[match(mydataSig_24h$AsterCLV3_24h$Gene,up_or_down$Gene)]
mydataSig_24h$AsterCLV3_L5S_24h$Direction<-up_or_down$x[match(mydataSig_24h$AsterCLV3_L5S_24h$Gene,up_or_down$Gene)]
mydataSig_24h$AtCLV3p_24h$Direction<-up_or_down$x[match(mydataSig_24h$AtCLV3p_24h$Gene,up_or_down$Gene)]
mydataSig_24h$AtCLV3p_S5L_24h$Direction<-up_or_down$x[match(mydataSig_24h$AtCLV3p_S5L_24h$Gene,up_or_down$Gene)]

# create a column in each df that contains a treatment variable
mydataSig_24h$AsterCLV3_24h<-cbind(mydataSig_24h$AsterCLV3_24h, Treatment="AsterCLV3_24h")
mydataSig_24h$AsterCLV3_L5S_24h<-cbind(mydataSig_24h$AsterCLV3_L5S_24h, Treatment="AsterCLV3_L5S_24h")
mydataSig_24h$AtCLV3p_24h<-cbind(mydataSig_24h$AtCLV3p_24h, Treatment="AtCLV3p_24h")
mydataSig_24h$AtCLV3p_S5L_24h<-cbind(mydataSig_24h$AtCLV3p_S5L_24h, Treatment="AtCLV3p_S5L_24h")
...
# combine all the dataframes on the rows
mydataSig_24h_full<-bind_rows(mydataSig_24h$AsterCLV3_24h, mydataSig_24h$AsterCLV3_L5S_24h,mydataSig_24h$AtCLV3p_24h,mydataSig_24h$AtCLV3p_S5L_24h)
mydataSig_24h_full[,"Gene"]
comparisons<-c("AsterCLV3_24h", "AsterCLV3_L5S_24h", "AtCLV3p_24h","AtCLV3p_S5L_24h")
treatment<-c("Treatment")

# one-hot encode the treatment variables -- why did I do this?
mydataSig_24h_full<-mutate(mydataSig_24h_full, AsterCLV3_24h=ifelse(Treatment=="AsterCLV3_24h",1,0))
mydataSig_24h_full<-mutate(mydataSig_24h_full, AsterCLV3_L5S_24h=ifelse(Treatment=="AsterCLV3_L5S_24h",1,0))
mydataSig_24h_full<-mutate(mydataSig_24h_full, AtCLV3p_24h=ifelse(Treatment=="AtCLV3p_24h",1,0))
mydataSig_24h_full<-mutate(mydataSig_24h_full, AtCLV3p_S5L_24h=ifelse(Treatment=="AtCLV3p_S5L_24h",1,0))


mydataSig_24h_subset<-mydataSig_24h_full[c("Gene","Direction","AsterCLV3_24h","AsterCLV3_L5S_24h","AtCLV3p_24h","AtCLV3p_S5L_24h")]
...
# get the treatment names from the columns
treatments<-colnames(mydataSig_24h_subset)[3:6]
# turn the binary to T or F
mydataSig_24h_subset[treatments]=mydataSig_24h_subset[treatments] ==1
# replace all F with NA
mydataSig_24h_subset<-mydataSig_24h_subset %>% replace_with_na_all(condition=~.x==FALSE)

# combine the columns where the gene ID is the same (keeping treatment identity intact)
mydataSig_24h_plot<-mydataSig_24h_subset %>% group_by(Gene)  %>% summarise_all(list(~ .[!is.na(.)][1]))
# convert the NA back to false (there has to be a better way to do this than
# converting the FALSE to NA, combining and then NA back to false...but I cannot figure it out)
mydataSig_24h_plot[is.na(mydataSig_24h_plot)] <- FALSE
# create a direction variable (stacked bar chart colored by column)
Direction<-colnames(mydataSig_24h_plot)[2]
# plot the upset 
png("plots/24h_up_AND_down.png", width=900, height=700
