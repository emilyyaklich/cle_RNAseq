...
mydataSig_24h_test <- list()

# Combine data frames with the same name
for (name in common_names) {
  mydataSig_24h_test[[name]] <- rbind(dataSigup_24h[[name]], dataSigdown_24h[[name]])
}
# Add the additional dataframe from dataSigdown_24h
mydataSig_24h_test$result_AsterCLV3_24h <- dataSigdown_24h$result_AsterCLV3_24h


# rename the dataframes in the list
names(mydataSig_24h_test) <-c("AsterCLV3_L5S_24h","AtCLV3p_24h","AsterCLV3_24h")

# subset the dataframes to only contain the gene ID and the direction of expression
gene_direction_only<-lapply(mydataSig_24h_test, function(x) {x[c("Gene", "Direction")]})
gene_direction_only<-gene_direction_only %>% purrr::reduce(full_join,by=c('Gene'))
# combine all three columns of direction into one string with where direction 
# is outlined. For example. "Up Up Down" If the gene is not in a treatment NA is used
...
#difference<-cbind(difference,x="Difference")
#colnames(difference)[1]<-"Gene"
# difference is EMPTY...none show a chance in direction that is why it is commented out
up<-data.frame(up)
up<-cbind(up,x="Up")
colnames(up)[1]<-"Gene"
down<-data.frame(down)
...
# combine all the dataframes on the rows
mydataSig_24h_full<-bind_rows(mydataSig_24h$AsterCLV3_24h, mydataSig_24h$AsterCLV3_L5S_24h,mydataSig_24h$AtCLV3p_24h)
...
mydataSig_24h_subset<-mydataSig_24h_full[c("Gene","Direction","AsterCLV3_24h","AsterCLV3_L5S_24h","AtCLV3p_24h")]
...
# get the treatment names from the columns
treatments<-colnames(mydataSig_24h_subset)[3:5]
# turn the binary to T or F
...
ComplexUpset::upset(mydataSig_24h_plot,
                    treatments,name="Treatment",
                    base_annotations=list
                    ("Number of Intersecting Genes"=intersection_size
                      (counts=TRUE,mapping=aes(fill=Direction))
